...
function renderWillDoScreen() {
  const container = document.getElementById('willdo-groups');
  const emptyEl = document.getElementById('willdo-empty-state');

  if (State.willDoItems.length === 0) {
    container.innerHTML = '';
...
async function handleToggleWillDoItem(id) {
  const item = State.willDoItems.find((w) => w.id === id);
  if (!item) return;
  item.completed = !item.completed;
  item.completedAt = item.completed ? new Date().toISOString() : null;
  await DB.WillDo.save(item);
  renderCurrentScreen();
}
...
/**
 * Add-item sheet for Will Do. Unlike the Task/Shopping modals, submitting
...
const WillDoModal = {
  ...
  sessionIds: [],
  init() {
    this.overlay = document.getElementById('willdo-modal-overlay');
  },
  open() {
    this.sessionIds = [];
    ...
    setTimeout(() => document.getElementById('willdo-item-field').focus(), 50);
  },
  hide() {
    this.overlay.hidden = true;
  },
  renderSessionList() {
    const list = document.getElementById('willdo-session-list');
    const items = this.sessionIds
      .map((id) => State.willDoItems.find((w) => w.id === id))
      .filter(Boolean);
    if (items.length === 0) {
      list.innerHTML = '<li class="checklist-empty">Nothing added yet this time.</li>';
      return;
    }
...
async function handleWillDoFormSubmit(e) {
  e.preventDefault();
  const textField = document.getElementById('willdo-item-field');
  const text = textField.value.trim();
  if (!text) {
    document.getElementById('willdo-item-error').hidden = false;
    textField.focus();
    return;
  }
  document.getElementById('willdo-item-error').hidden = true;
  ...
  await DB.WillDo.save(item);
  State.willDoItems.push(item);
  WillDoModal.sessionIds.push(item.id);
  WillDoModal.renderSessionList();
...
function switchScreen(screenName) {
  ...
  updateGlobalAddButtonVisibility();
  renderCurrentScreen();
...
/**
 * The global "+ Add Task" entry points (the mobile FAB and the desktop
 * sidebar button) only make sense on the Today and Planner screens — every
 * other screen either has nothing to add (Vision, Stats, More) or has its
 * own dedicated add button already (Buy, Will Do).
 */
function updateGlobalAddButtonVisibility() {
  const show = State.currentScreen === 'today' || State.currentScreen === 'planner';
  document.getElementById('fab-add-btn').hidden = !show;
  document.getElementById('sidebar-add-btn').hidden = !show;
}
...
function renderCurrentScreen() {
  switch (State.currentScreen) {
    ...
    case 'willdo': renderWillDoScreen(); break;
...
async function loadAllData() {
  ...
  State.willDoItems = await DB.WillDo.getAll();
...
async function seedSampleData() {
  ...
  const now = new Date().toISOString();
  const sample = [
    { title: 'Morning walk', time: '07:00', priority: 'low' },
    { title: 'Pay electricity bill', time: '10:00', priority: 'high' },
    { title: 'Call insurance company', time: '14:30', priority: 'medium' },
    { title: 'Buy groceries', time: '18:00', priority: 'medium' },
    { title: 'Review investments', time: '20:00', priority: 'low' },
  ];
...
const TaskModal = {
  overlay: null,
  checklistDraft: [], // [{ id, text, completed }] — working copy while the sheet is open
  init() {
  ...
  resetForm() {
    ...
    document.getElementById('checklist-new-item-field').value = '';
  },
  setPriority(value) {
  ...
  renderChecklist() {
    ...
    if (this.checklistDraft.length === 0) {
      list.innerHTML = '<li class="checklist-empty">No checklist items yet.</li>';
      return;
    }
    list.innerHTML = this.checklistDraft.map((item) => `
      <li class="checklist-item ${item.completed ? 'is-completed' : ''}" data-item-id="${item.id}">
        <button type="button" class="checklist-item-checkbox" data-action="toggle-checklist-item" aria-label="${item.completed ? 'Mark not done' : 'Mark done'}">${item.completed ? '✓' : ''}</button>
        <span class="checklist-item-text">${escapeHTML(item.text)}</span>
        <button type="button" class="checklist-item-remove" data-action="remove-checklist-item" aria-label="Remove item">✕</button>
      </li>`).join('');
  },
...
async function handleTaskFormSubmit(e) {
  ...
  const task = {
    ...
    title,
    checklist: TaskModal.checklistDraft.map((item) => ({ ...item })),
...
function setupEventListeners() {
  ...
  // Will Do: add-item entry points, form, session-preview removal, and
  // main-list toggle/remove (delegated on document, like tasks/shopping).
  document.getElementById('willdo-add-btn').addEventListener('click', () => WillDoModal.open());
  document.getElementById('willdo-empty-add-btn').addEventListener('click', () => WillDoModal.open());
  document.getElementById('willdo-modal-close-btn').addEventListener('click', () => WillDoModal.hide());
  document.getElementById('willdo-done-btn').addEventListener('click', () => WillDoModal.hide());
  document.getElementById('willdo-modal-overlay').addEventListener('click', (e) => {
    if (e.target.id === 'willdo-modal-overlay') WillDoModal.hide();
  });
  document.getElementById('willdo-form').addEventListener('submit', handleWillDoFormSubmit);
  document.getElementById('willdo-session-list').addEventListener('click', (e) => {
    const btn = e.target.closest('[data-action="remove-willdo-session-item"]');
    if (!btn) return;
  ...
  document.addEventListener('click', (e) => {
    ...
    const willdoCheckbox = e.target.closest('[data-action="toggle-willdo"]');
    if (willdoCheckbox) {
      e.stopPropagation();
      const card = willdoCheckbox.closest('.willdo-item-card');
      handleToggleWillDoItem(card.dataset.willdoId);
      return;
    }
    const willdoRemove = e.target.closest('[data-action="remove-willdo"]');
    if (willdoRemove) {
      e.stopPropagation();
      const card = willdoRemove.closest('.willdo-item-card');
      handleRemoveWillDoItem(card.dataset.willdoId);
      return;
    }
  ...
  // Checklist (sub-tasks) builder inside the task form
  document.getElementById('checklist-add-btn').addEventListener('click', () => {
    const field = document.getElementById('checklist-new-item-field');
    TaskModal.addChecklistItem(field.value);
    field.value = '';
    field.focus();
  });
  document.getElementById('checklist-new-item-field').addEventListener('keydown', (e) => {
    if (e.key !== 'Enter') return;
    e.preventDefault(); // don't submit the task form on Enter
  ...
  document.getElementById('checklist-items').addEventListener('click', (e) => {
    ...
    if (e.target.closest('[data-action="toggle-checklist-item"]')) {
      TaskModal.toggleChecklistItem(itemId);
    } else if (e.target.closest('[data-action="remove-checklist-item"]')) {
      TaskModal.removeChecklistItem(itemId);
    }
  });
