 ...
 *
 * Everything the app stores lives in one IndexedDB database with four
 * object stores: `tasks`, `shopping`, `willdo`, and `settings`. This file
 * exposes a small promise-based API (`DB.*`) so the rest of the app never
 * has to touch IndexedDB transactions directly.
 */
...
function openDB() {
  ...
  return new Promise((resolve, reject) => {
    ...
    request.onupgradeneeded = (event) => {
      ...
      if (!db.objectStoreNames.contains(STORE_WILLDO)) {
        const willDoStore = db.createObjectStore(STORE_WILLDO, { keyPath: 'id' });
        willDoStore.createIndex('date', 'date', { unique: false });
        willDoStore.createIndex('completed', 'completed', { unique: false });
      }
...
const WillDo = {
  async getAll() {
    return getAll(STORE_WILLDO);
  },
  async get(id) {
    return getOne(STORE_WILLDO, id);
  },
  async save(item) {
    return putOne(STORE_WILLDO, item);
  },
  async remove(id) {
    return deleteOne(STORE_WILLDO, id);
  },
  async clear() {
    return clearStore(STORE_WILLDO);
  },
  newId() {
    return generateId('willdo');
  },
};
...
async function exportAllData() {
  const [tasks, shopping, willdo, settings] = await Promise.all([
    Tasks.getAll(),
    Shopping.getAll(),
    WillDo.getAll(),
    Settings.getAll(),
  ...
  return {
    ...
    version: DB_VERSION,
    data: { tasks, shopping, willdo, settings },
  };
}

async function importAllData(payload) {
  if (!payload || !payload.data) throw new Error('Invalid backup file');
  const { tasks = [], shopping = [], willdo = [], settings = {} } = payload.data;

  await Promise.all([clearStore(STORE_TASKS), clearStore(STORE_SHOPPING), clearStore(STORE_WILLDO)]);
  ...
  const willDoStore = await tx(STORE_WILLDO, 'readwrite');
  await Promise.all(willdo.map((w) => promisifyRequest(willDoStore.put(w))));
